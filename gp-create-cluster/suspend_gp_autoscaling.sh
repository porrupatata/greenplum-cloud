...
echo -e "\n SUSPENDING AUTOSCALING FOR $greenplum_cluster \n"
aws autoscaling  describe-auto-scaling-groups --query 'AutoScalingGroups[].[AutoScalingGroupName]' --output text| grep "$greenplum_cluster"-M |xargs aws autoscaling suspend-processes --scaling-processes Launch ScheduledActions AddToLoadBalancer HealthCheck --auto-scaling-group-name

aws autoscaling  describe-auto-scaling-groups --query 'AutoScalingGroups[].[AutoScalingGroupName]' --output text| grep "$greenplum_cluster"-S |xargs aws autoscaling suspend-processes --scaling-processes Launch ScheduledActions AddToLoadBalancer HealthCheck  --auto-scaling-group-name
